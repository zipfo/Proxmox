...
function header_info {
  clear
  cat <<"EOF"
    ____                               __        ___       ______  ______
   /  _/___ ___  ____ ___  ____  _____/ /_____ _/ / |     / / __ \/_  __/
   / // __ `__ \/ __ `__ \/ __ \/ ___/ __/ __ `/ /| | /| / / /_/ / / /   
 _/ // / / / / / / / / / / /_/ / /  / /_/ /_/ / / | |/ |/ / _, _/ / /    
/___/_/ /_/ /_/_/ /_/ /_/\____/_/   \__/\__,_/_/  |__/|__/_/ |_| /_/     
...
if (whiptail --backtitle "Proxmox VE Helper Scripts" --title "ImmortalWrt VM" --yesno "This will create a New ImmortalWrt VM. Proceed?" 10 58); then
  :
else
...
function default_settings() {
  ...
  echo -e "${BL}Creating a ImmortalWrt VM using the above default settings${CL}"
}
...
function advanced_settings() {
  ...
  if VM_NAME=$(whiptail --backtitle "Proxmox VE Helper Scripts" --inputbox "Set Hostname" 8 58 immortalwrt --title "HOSTNAME" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
    if [ -z $VM_NAME ]; then
      HN="immortalwrt"
    else
  ...
  if (whiptail --backtitle "Proxmox VE Helper Scripts" --title "ADVANCED SETTINGS COMPLETE" --yesno "Ready to create ImmortalWRT VM?" --no-button Do-Over 10 58); then
    echo -e "${RD}Creating a ImmortalWRT VM using the above advanced settings${CL}"
  else
    header_info
...
if [ -z "$VALID" ]; then
  ...
  while [ -z "${STORAGE:+x}" ]; do
    STORAGE=$(whiptail --backtitle "Proxmox VE Helper Scripts" --title "Storage Pools" --radiolist \
      "Which storage pool you would like to use for the ImmortalWRT VM?\n\n" \
...
msg_info "Getting URL for ImmortalWRT Disk Image"

response=$(curl -s https://downloads.immortalwrt.org/)
stableversion="23.05.4"
...
msg_ok "Extracted & Resized ImmortalWRT Disk Image ${CL}${BL}$FILE${CL}"
...
msg_info "Creating ImmortalWRT VM"
...
msg_ok "Created ImmortalWRT VM ${CL}${BL}(${HN})"
msg_info "ImmortalWRT is being started in order to configure the network interfaces."
qm start $VMID
sleep 15
msg_ok "Network interfaces are being configured as ImmortalWRT initiates."
send_line_to_vm ""
...
if [ "$START_VM" == "yes" ]; then
  msg_info "Starting ImmortalWRT VM"
  qm start $VMID
  msg_ok "Started ImmortalWRT VM"
fi
VLAN_FINISH=""
